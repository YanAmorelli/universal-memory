...
class MCPClient:
    ...
    def read_response(self, expected_id, timeout=5):
        ...
        raise TimeoutError(
            f"Resposta com id {expected_id} nao recebida dentro de {timeout} segundos. Logs: {self.log_messages}"
        )

    def call_tool(self, tool_name, arguments):
        req_id = self.send_request("tools/call", {"name": tool_name, "arguments": arguments})
    ...
    def initialize(self):
        req_id = self.send_request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "alpha-tester-v2", "version": "2.0"},
            },
        )
    ...
    if expect_fail:
        if result.returncode == 0:
            raise AssertionError(
                f"Esperava que o comando falhasse, mas retornou 0: {' '.join(cmd)}\nStdout: {result.stdout}\nStderr: {result.stderr}"
            )
...
def main():
    ...
    try:
        ...
        report.append(
            "| 1. Preparacao | ✅ PASS | Sandbox criada, `umem --help` executado com sucesso. |"
        )
    ...
    try:
        ...
        assert status_data.get("initialized") is False, (
            "Esperava initialized=False antes de rodar o init"
        )

        stdout, _, _ = run_cli(
            ["init", "--yes", "--hosts", "codex", "--hosts", "claude_code", "--format", "json"],
            project_path,
            env,
        )
        ...
        assert init_envelope_2.get("ok") is True or init_envelope_2.get("status") == "success", (
            "Init de idempotencia falhou"
        )

        stdout, _, _ = run_cli(["status", "--format", "json"], project_path, env)
        envelope_2 = json.loads(stdout)
        status_data_2 = envelope_2.get("data", {})
        assert status_data_2.get("initialized") is True, (
            "A idempotencia quebrou o status inicializado"
        )

        report.append(
            "| 2. Smoke CLI Basico | ✅ PASS | Inicializacao bem sucedida, idempotencia confirmada, layout verificado. |"
        )
    except Exception as e:
        traceback.print_exc()
    ...
    try:
        ...
        run_cli(
            [
                "remember",
                "Preferir respostas objetivas em portugues.",
                "--scope",
                "global",
                "--tag",
                "preference",
        ...
        stdout_local, _, _ = run_cli(
            ["facts", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        facts_local = json.loads(stdout_local).get("data", {}).get("facts", [])
        assert any("arquitetura hexagonal" in f.get("content", "") for f in facts_local), (
            "Fato local nao encontrado na listagem"
        )

        stdout_global, _, _ = run_cli(
            ["facts", "list", "--scope", "global", "--format", "json"], project_path, env
        )
        facts_global = json.loads(stdout_global).get("data", {}).get("facts", [])
        assert any("portugues" in f.get("content", "") for f in facts_global), (
            "Fato global nao encontrado na listagem"
        )

        stdout_ctx, _, _ = run_cli(
            ["context", "--scope", "project", "--max-size-chars", "4000", "--format", "json"],
            project_path,
            env,
        )
        ctx_envelope = json.loads(stdout_ctx)
        assert ctx_envelope.get("ok") is True and "project_summary" in ctx_envelope.get(
            "data", {}
        ), "Falha ao gerar contexto local"

        stdout_ctx_glob, _, _ = run_cli(
            ["context", "--scope", "global", "--format", "json"], project_path, env
        )
        ctx_glob_envelope = json.loads(stdout_ctx_glob)
        assert ctx_glob_envelope.get("ok") is True and "project_summary" in ctx_glob_envelope.get(
            "data", {}
        ), "Falha ao gerar contexto global"

        report.append(
            "| 3. Memoria Local e Global | ✅ PASS | Fatos gravados e recuperados corretamente em ambos os escopos. Contextos gerados com sucesso. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        stdout_snapshots, _, _ = run_cli(
            ["snapshots", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        snapshots_envelope = json.loads(stdout_snapshots)
        assert len(snapshots_envelope.get("data", {}).get("snapshots", [])) > 0, (
            "Deveria haver pelo menos 1 snapshot"
        )

        stdout_audit, _, _ = run_cli(
            ["audit", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        audit_envelope = json.loads(stdout_audit)
        assert len(audit_envelope.get("data", {}).get("events", [])) > 0, (
            "Deveria haver eventos no log de auditoria"
        )
        ...
        report.append(
            "| 4. Seguranca (Deteccao Segredo) | ✅ PASS | Segredo de alta entropia detectado e bloqueado com sucesso. |"
        )

        run_cli(
            ["remember", "Fato antes do rollback.", "--scope", "project", "--format", "json"],
            project_path,
            env,
        )
        stdout_f1, _, _ = run_cli(
            ["facts", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        facts_f1 = json.loads(stdout_f1).get("data", {}).get("facts", [])

        run_cli(["rollback", "--scope", "project", "--yes", "--format", "json"], project_path, env)
        stdout_f2, _, _ = run_cli(
            ["facts", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        ...
        report.append(
            "| 4. Snapshots e Rollback | ✅ PASS | Snapshots gravados, auditados e rollback executado com sucesso. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        ...
        run_cli(
            ["facts", "purge", "--id", fact_id, "--format", "json"],
            project_path,
            env,
            expect_fail=True,
        )
        ...
        stdout_list_2, _, _ = run_cli(
            ["facts", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        facts_list_2 = json.loads(stdout_list_2).get("data", {}).get("facts", [])
        assert not any(f["id"] == fact_id for f in facts_list_2), (
            "Fato purgado ainda aparece na listagem ativa"
        )

        run_cli(["facts", "hygiene", "--yes", "--format", "json"], project_path, env)
        report.append(
            "| 5. Purge e Hygiene | ✅ PASS | Purge destructivo protegido, purge apaga fisicamente o dado por seguranca e hygiene executa sem erro. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        ...
        stdout_check_codex, _, _ = run_cli(
            ["host", "check", "codex", "--format", "json"], project_path, env
        )
        check_codex_envelope = json.loads(stdout_check_codex)
        assert (
            check_codex_envelope.get("ok") is True
            or check_codex_envelope.get("data", {}).get("status") == "success"
        ), "Check do codex falhou"

        run_cli(["host", "sync", "--no-apply", "--format", "json"], project_path, env)
        run_cli(["host", "sync", "--apply", "--yes", "--format", "json"], project_path, env)

        report.append(
            "| 6. Hosts | ✅ PASS | Setup, Check e Sync dos hosts codex e claude_code executados com exito. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        ...
        assert any(s["name"] == latent_skill_data["name"] for s in skills_list), (
            "Latent skill da fixture nao listada"
        )

        run_cli(
            ["skills", "propose", latent_skill_id, "--decision", "yes", "--format", "json"],
            project_path,
            env,
        )
        run_cli(
            ["skills", "generate", latent_skill_id, "--yes", "--format", "json"], project_path, env
        )
        assert (project_path / ".umem" / "skills" / "autorefactor" / "SKILL.md").is_file(), (
            "Estrutura fisica da skill nao foi gerada"
        )
        ...
        report.append(
            "| 7. Skills | ✅ PASS | Proposta, geracao, ativacao, desativacao e atualizacao de skills via CLI validados. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        ...
        assert (
            data_status.get("ok") is True or data_status.get("data", {}).get("initialized") is True
        ), "Status do MCP falhou"
        ...
        assert data_rem.get("ok") is True or "id" in data_rem.get("data", {}).get("fact", {}), (
            "remember_fact no MCP falhou"
        )

        resp = mcp.call_tool("list_facts", {"scope": "project"})
        data_list = get_mcp_data(resp)
        facts = data_list.get("data", {}).get("facts", [])
        assert any("MCP grava fatos" in f.get("content", "") for f in facts), (
            "Fato gravado por MCP nao aparece na listagem do MCP"
        )
        ...
        assert len(data_snap.get("data", {}).get("snapshots", [])) > 0, (
            "list_snapshots falhou no MCP"
        )

        resp = mcp.call_tool("list_audit_events", {"scope": "project"})
        data_audit = get_mcp_data(resp)
        assert len(data_audit.get("data", {}).get("events", [])) > 0, (
            "list_audit_events falhou no MCP"
        )
        ...
        assert any(s["name"] == latent_skill_data_mcp["name"] for s in skills), (
            "Latent skill do MCP nao listada"
        )

        resp = mcp.call_tool(
            "propose_skill", {"latent_skill_id": latent_skill_id_mcp, "decision": "yes"}
        )
        data_prop = get_mcp_data(resp)
        assert data_prop.get("ok") is True, "propose_skill falhou no MCP"

        resp = mcp.call_tool(
            "generate_skill", {"latent_skill_id": latent_skill_id_mcp, "update_existing": False}
        )
        data_gen = get_mcp_data(resp)
        assert data_gen.get("ok") is True, "generate_skill falhou no MCP"
        assert (project_path / ".umem" / "skills" / "mcpskill" / "SKILL.md").is_file(), (
            "Skill gerada pelo MCP nao existe no filesystem"
        )
        ...
        report.append(
            "| 8. MCP Black-Box | ✅ PASS | Todas as 19 tools MCP testadas de forma Black-Box com sucesso. Envelopes e schemas validados. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
    ...
    try:
        ...
        assert any("Criado pela CLI, lido pelo MCP" in f.get("content", "") for f in facts_mcp), (
            "Fato da CLI nao lido pelo MCP"
        )

        resp_rem2 = mcp2.call_tool(
            "remember_fact", {"content": "Criado pelo MCP, lido pela CLI", "scope": "project"}
        )
        mcp2.close()

        stdout_cli, _, _ = run_cli(
            ["facts", "list", "--scope", "project", "--format", "json"], project_path, env
        )
        facts_cli = json.loads(stdout_cli).get("data", {}).get("facts", [])
        assert any("Criado pelo MCP, lido pela CLI" in f.get("content", "") for f in facts_cli), (
            "Fato do MCP nao lido pela CLI"
        )

        report.append(
            "| 9. Compatibilidade CLI/MCP | ✅ PASS | CLI e MCP compartilham o mesmo storage de forma consistente e transparente. |"
        )
    except Exception as e:
        traceback.print_exc()
        err_msg = "".join(traceback.format_exception_only(type(e), e)).strip()
