...
11. **Given** an AGENTS.md/Codex/OpenAI-class host is enabled and supports native Agent Skills, **When** sync runs, **Then** `.agents/skills/<slug>/SKILL.md` is treated as a configured native runtime target and receives a complete copy from `.umem/skills/<slug>/`, not a wrapper.
...
- **Complete native copies:** Sync writes complete skill directories from canonical into configured native targets. `.agents/skills/<slug>/SKILL.md` is a native target for enabled AGENTS.md/Codex/OpenAI-class hosts that support Agent Skills. Wrappers are an explicit local policy exception, not the default sync behavior.
