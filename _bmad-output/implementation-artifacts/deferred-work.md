...
## Deferred from: UMEM update and native skill adoption gaps (2026-07-08)

- Improvement: add a post-upgrade diagnostic, either as `umem update check` or as part of `umem doctor`, that warns when the installed UMEM package is newer than managed artifacts materialized inside the project. The diagnostic should especially detect stale `.umem/skills/use-universal-memory/` templates and guide the operator to `umem update --skills`.
- Documentation: publish the expected post-upgrade flow: update the package, run `umem update --skills --yes --format json`, run `umem host sync --apply --yes --format json`, then validate with `umem status` and `umem skills detail use-universal-memory`. Make the boundary explicit: package managers update CLI/code, while `umem update --skills` updates managed project artifacts.
- Documentation: add a short "Adopt existing native skill into UMEM" recipe. Minimum flow: confirm the target repository, run UMEM bootstrap in that repository, run `umem skills import`, optionally convert the native copy into a wrapper if the repository has an explicit wrapper policy, validate with `umem skills detail`, record durable memory when appropriate, and run `umem host sync --apply --yes --format json`.
- Documentation: make the authoring rule explicit: existing skills from `.agents/`, `.opencode/`, or other native directories should use `umem skills import`; brand-new skills should use `umem skills create`.
- Documentation: clarify that an import result with empty `native_installations` and empty `targets` can still mean the skill was adopted into the canonical UMEM store, but the original native path is not being managed as a sync target. Also clarify the difference between canonical skill storage, synchronized native targets, and wrapper compatibility files.
- Improvement: improve `umem skills list` or add a diagnostic to detect native skills in `.agents/`, `.opencode/`, and other configured targets that are not registered in UMEM yet.
- Bug or documentation gap: investigate the perceived mismatch where `umem status` reported `registered_skills_count` as 1 while `umem skills list` showed several active skills. Either explain the metric clearly, including scope and active-state semantics, or correct the count.
- Improvement: warn when paths created or modified by `umem skills import` or `umem update --skills` are ignored by Git, because `git status` can look clean even though managed artifacts changed. Recommend validation with `umem skills detail` and direct file inspection.
- Documentation: make non-interactive examples consistent by pairing `--format json` with `--yes` for apply-style commands such as `umem host sync --apply --yes --format json`.
- Documentation: in multi-repository tasks, state explicitly that UMEM bootstrap must run in the target repository, not only in the initial conversation working directory.
- Product direction: consider a declarative flow such as `umem skills migrate PATH --scope project --wrap-source --sync --warn-git-ignore`, plus a focused `umem skills doctor NAME`. The goal is to reduce the manual sequence of import, canonical validation, wrapper or target policy decisions, runtime sync, Git-ignore warnings, and final drift checks.
- Cleanup: reduce duplicate instruction warnings from `umem host sync --apply --yes --format json` when both `AGENTS.md` and `CLAUDE.md` contain equivalent UMEM bootstrap or proactive memory loop lines. The warnings do not block skill behavior, but they create noise and can hide more important sync warnings.
